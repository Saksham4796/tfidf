...
for i in range(0,len(x[0])):
	...
	for j in range(0,len(y)):
		...
		if x[j][i]==1 and y[j][0]==0:
			...
			nfailure=nfailure+1
			flag=1
		elif x[j][i]==0 and y[j][0]==1:
			flag=0
			break
		elif x[j][i]==0 and y[j][0]==0:
			flag=1
	try:
		if flag==1:
			...
			sus_score=float(nfailure)/float((total_failed+nsuccess+1000*(float((total_failed-nfailure)*nsuccess)/float(nfailure))))
...
csvfile=open(str_cwd+"/zoltar_unique_slicing.csv", "a+")
